...
class UNetModel(nn.Module):
    ...
    :param conv_resample: if True, use learned convolutions for upsampling and
        downsampling.
    :param dims: determines if the signal is 1D, 2D, or 3D.
    :param conditioning_dim: if specified (as an int), then this model will be
        conditional on a vector of length `conditioning_dim`.
    ...
    conditioning_on_sequence: bool = False
    ...
    def __post_init__(self):
        ...
        if self.conditioning_dim is not None:
            self.conditioning_emb = nn.Linear(
                self.conditioning_dim, self.time_embed_dim
            )
            
            if self.conditioning_on_sequence:
                compression_dim = 256
                self.activation_down_proj = nn.Linear(
                    self.conditioning_dim,
                    compression_dim
                ).to('cuda')
                
                self.activation_up_proj = nn.Linear(
                    compression_dim,
                    self.conditioning_dim
                ).to('cuda')
                ...
                encoder_layer = nn.TransformerEncoderLayer(
                    d_model=compression_dim,
                    nhead=4,
                    dim_feedforward=compression_dim,
                    batch_first=True
                ).to('cuda')
                ...
                self.latent_processor = nn.Linear(
                    compression_dim * 301,
                    self.conditioning_dim
                )

                self.basic_linear = nn.Linear(
                    100 * 301,
                    self.conditioning_dim
                )
                
                self.basic_linear2 = nn.Linear(
                    self.conditioning_dim,
                    self.conditioning_dim
                )
        ...
        emb_off = self.ignore_time and self.conditioning_dim is None
    ...
    def forward(self, x, timesteps, conditioning):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        :param conditioning: an optional [N x d] Tensor of conditioning variables or 
            [N x d x T] Tensor of conditioning sequences.
        :return: an [N x C x ...] Tensor of outputs.
        """
        if self.with_fourier_features:
        ...
        if self.conditioning_dim and (conditioning is None):
            # Hack to deal with ddp find_unused_parameters not working with activation checkpointing...
            conditioning = torch.zeros(
                (x.size(0), self.conditioning_dim), device=x.device
            )

        if self.conditioning_dim and (conditioning is not None):
            assert conditioning.shape[0] == x.shape[0]
            if self.conditioning_on_sequence:
                assert conditioning.ndim == 3

                # This works!
                # conditioning_1 = conditioning[:, 0, 0]
                # conditioning_2 = conditioning[:, 1, 0]
                # conditioning = conditioning[:, -1, :]
                # conditioning[:, 0:100] = conditioning_1.unsqueeze(1)
                # conditioning[:, 100:200] = conditioning_2.unsqueeze(1)
                # conditioning = self.basic_linear2(conditioning)

                # This doesn't work! (except with lr of 0.0001)
                # conditioning = conditioning.clamp(min=-1.0, max=1.0)
                # conditioning = self.activation_down_proj(conditioning)
                # conditioning = rearrange(conditioning, 'b s d -> b (s d)')
                # conditioning = self.latent_processor(conditioning)
                
                # This didn't work!
                # conditioning = conditioning.clamp(min=-1.0, max=1.0)
                # conditioning = conditioning[:, :, :100]
                # conditioning = rearrange(conditioning, 'b s d -> b (s d)')
                # conditioning = self.basic_linear(conditioning)


                # conditioning = conditioning.clamp(min=-1.0, max=1.0)
                conditioning = self.activation_down_proj(conditioning)
                conditioning = self.positional_encoding(conditioning)
                encoded = self.transformer_encoder(conditioning)
                conditioning = reduce(encoded, 'b s d -> b d', 'mean')
                conditioning = self.activation_up_proj(conditioning)

            else:
                assert conditioning.ndim == 2
            emb = emb + self.conditioning_emb(conditioning)
            
        if (
            (conditioning is not None) and
            (conditioning.shape[1] > 0) and
            (not self.conditioning_dim)
        ):
            raise ValueError("Conditioning with no conditioning dimension")
